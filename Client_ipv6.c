...
struct sockaddr_in6 serveraddr; //Server address
...
void * scanf_thread1_f() {
	...
	while (1) {
		...
		if (memcmp(txbuffer, "exit", 4) == 0) {
			...
			//Send data to server
			rc = write(sd, txbuffer, strlen(txbuffer)+1);	//Include '\0' in the string
			if (rc < 0) {
...
int main(int argc, char *argv[]) {
	...
	//use the gethostbyname2() function to retrieve the address of the host server.
	hostp = gethostbyname2(server,AF_INET6);
	...
	memcpy(&serveraddr.sin6_addr, hostp->h_addr, sizeof(serveraddr.sin6_addr));
